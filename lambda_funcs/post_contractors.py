...
def handler(event, context) -> Dict[str, Any]:
    ...
    # user_id
    # Custom user name
    # TODO: ABC123: This will come from Incognito in final version
    if event["queryStringParameters"].get("user_id") is None:
        raise KeyError("user_id not included in queryStringParameters")
    try:
        user_id = event["queryStringParameters"].get('user_id')
        user_id_dict = {"longValue": int(user_id)}
        user_param = {"name": "user_id", "value": user_id_dict}
    except Exception as e:
        print("Could not validate user_id")
        print(e)
    ...
    minimum_service_charge_cents_param = {"name": "minimum_service_charge_cents", "value": minimum_service_charge_cents_dict}
